...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      selectInput("predictors", "Choose predictors:",
                  choices = names(mtcars)[!names(mtcars) %in% "mpg"],
                  selected = c("disp","hp","wt"),
                  multiple = TRUE),
...
server <- function(input, output, session) {
  # reactive train/test split, update when resample pressed
  split <- eventReactive(input$resample, {
    make_split(mtcars, prop = 0.7)
  }, ignoreNULL = FALSE)
  
  # --- Build model dynamically based on selected predictors ---
  model <- reactive({
    d <- split()$train
    req(input$predictors)
    form <- as.formula(paste("mpg ~", paste(input$predictors, collapse = "+")))
    knn_s3(form, data = d, k = input$k)
  })
  
  # compute fitted (train) and predicted (test) values
